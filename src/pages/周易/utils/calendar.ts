...
export function formatBazi(mode: CalendarMode, parts: BirthParts, shichen: string, timestamp: number) {
  ...
  const hour = shichenLabel(shichen)
  if (mode === 'solar') return `${picked} ${hour}`
  const solar = solarPartsFromTimestamp(timestamp)
  if (!solar) return `${picked} ${hour}`
  return `${picked}（公历${solar.year}年${solar.month}月${solar.day}日） ${hour}`
}
