...
import { SHICHEN, t } from '../utils/i18n'
...
export default function BirthPicker({
  ...
  const hourOptions = useMemo(
    () => SHICHEN.map((item, index) => ({ value: index, label: item.name })),
    [],
  )
  ...
  const birthEcho = value
    ? `${value.calendar === 'solar' ? '公历' : ''}${formatDateLabel(value.calendar, value.birth)} ${value.shichen}`
    : ''
  const draftHourItem = SHICHEN[draftHour]
  const draftEcho =
    `${draftCalendar === 'solar' ? '公历' : ''}${formatDateLabel(draftCalendar, resolvedDraft())} ${draftHourItem?.name ?? ''}`.trim()
...
const Style = styled.div`
  ...
  .sheet-hour-range {
    margin: 12px 0 0;
    color: #f6edd8;
    font: 700 24px/1.45 'Songti SC', 'Noto Serif SC', serif;
  ...
  .picker-trigger {
    ...
    color: #f6edd8;
    font: 16px/1.4 'Songti SC', 'Noto Serif SC', serif;
    letter-spacing: 0.08em;
    text-align: left;
    cursor: pointer;
    touch-action: manipulation;
  ...
  .date-wheel-columns {
    ...
    z-index: 2;
    display: grid;
    grid-template-columns: 1.2fr 1.05fr 0.95fr 0.8fr;
    height: 100%;
    min-height: 0;
    overflow: hidden;
  }
  ...
  .wheel-item {
    ...
    gap: 2px;
    color: rgba(232, 220, 198, 0.38);
    font: 18px/1 'Songti SC', 'Noto Serif SC', serif;
